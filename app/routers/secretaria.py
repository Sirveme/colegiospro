...
async def comunicado_enviar(
    ...
    try:
        ...
        urg = bool(urgente)
        # URL destino del push. NUNCA usar request.url ni str(request.url).
        # Fuente de verdad: solo los dos campos del form.
        _raw_destino = (url_destino or "").strip()
        _raw_custom = (url_custom or "").strip()
        _log_subs.info(
            "URL raw form → url_destino=%r url_custom=%r",
            _raw_destino, _raw_custom,
        )
        if _raw_destino == "custom":
            url_sel = _raw_custom or "/secretaria/muro"
        elif _raw_destino:
            url_sel = _raw_destino
        elif _raw_custom:
            url_sel = _raw_custom
        else:
